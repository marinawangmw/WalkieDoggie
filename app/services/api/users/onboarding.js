...
export const onBoardingWalker = (paramsOnBoardingWalker, id) => {
    const { address, profile_photo_uri, phone, cover_letter = '', ranges, price_per_hour } = paramsOnBoardingWalker;
    const config = {
    ...
    return privateRequest(config).then(data => ({ result: true, data }))
